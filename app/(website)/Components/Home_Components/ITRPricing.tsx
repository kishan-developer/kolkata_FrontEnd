...
import { Zap, TrendingUp, Building2, ShieldCheck } from "lucide-react";
...
const pricingPlans = [
  {
    ...
    hook: "Perfect for startups. Automatic filing with zero penalties.",
    price: "₹999 / month",
    popular: false
  },
  {
...
export default function ITRPricing() {
  ...
  return (
    <section className="w-full py-10 md:py-24 bg-gradient-to-b from-slate-50 to-white">
      ...
      >
        {/* Header */}
        <motion.div className="text-center mb-8 md:mb-12 space-y-4" variants={itemVariants}>
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-blue-50 rounded-full border border-blue-100">
            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[#2663eb]">Pricing</span>
          </div>
          <h2 className="text-3xl md:text-5xl font-black text-slate-900 tracking-tight leading-[1.1]">
            GST Compliance <span className="text-[#2663eb]">Monthly/Quarterly</span>
          </h2>
        ...
        {/* <motion.div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6" variants={itemVariants}>
          {pricingPlans.map((plan) => {
            ...
            return (
              <motion.div
                key={plan.id}
                className={`relative p-4 md:p-8 rounded-2xl border-2 ${
                  plan.popular
              ...
              >
                ...
                <button className={`w-full mt-6 py-3 rounded-xl font-black text-xs uppercase tracking-wider transition-all ${
                  plan.popular
